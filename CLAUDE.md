...
**Vídeo reproduzível (re-encode no `publish-report`):** antes do `allure generate`, o job re-encoda **incondicionalmente todos** os `.mp4` (Android e iOS) para H.264 + `+faststart`, com `-crf 28 -maxrate 2M` e downscale para no máximo 720p, mantendo o mesmo nome (a referência no `*-result.json` continua válida). Dois motivos distintos: o iOS (Appium 1.x) grava em MJPEG (`codec_tag mp4v`), que o navegador não toca em `<video>`; e o Android, mesmo já sendo h264, gerava arquivos acima do limite de 100 MB por arquivo do GitHub, que eram apagados no publish e viravam 404 no relatório. Por isso o passo **não** pula mais o que já é h264. O step é `continue-on-error` e cada arquivo que falhar mantém o original.
